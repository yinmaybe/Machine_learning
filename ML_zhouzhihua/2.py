...
def visualizeFit(X, mu, sigma2):
    ...
    if np.sum(np.isinf(Z).astype(float)) == 0:  # 如果计算的为无穷，就不用画了
        ...
        CS = plt.contour(X1, X2, Z, 10. ** np.arange(-20, 0, 3), color='black',
                         linewidth=.5)  # 画等高线，Z的值在10.**np.arange(-20, 0, 3)
        # plt.clabel(CS)
...
if __name__ == '__main__':
    anomalyDetection_example()#-*- coding: utf-8 -*-
# Author: Bob
# Date:   2016.12.22
import numpy as np
from matplotlib import pyplot as plt
